...
update_url = "1.1"
# update_url = requests.get(
#     "https://github.com/akula993/elukalin2/blob/master/Elukalin/version.txt"
# )
...
def main(page: ft.Page):
    ...
    page.add(img)
    img2 = ft.Image(
        src=f"assets/images/DoubleV.jpg", fit=ft.ImageFit.CONTAIN, tooltip="doublev.ru"
    )
    ...
    def button_a4():
        ...
        pdf = LableElukalinA4(
            orientation="L"
        )  # Устанавливаем альбомную ориентацию (L - landscape)
        pdf.add_page()
        pdf.top_right()
        pdf.end_right()
        ...
        pdf.contour()
        pdf.add_content(
            charge.value, nettogewicht.value, versanddatum.value, number.value
        )
        # Сохраняем PDF-файл
    ...
    charge = ft.TextField(
        label="Введите номер партии", hint_text="номер партии", helper_text=""
    )
    nettogewicht = ft.TextField(
        label="Введите вес нетто, например: 12", hint_text="вес нетто"
    )
    versanddatum = ft.TextField(
        label="Введите дату отгрузки", hint_text="дату отгрузки"
    )
